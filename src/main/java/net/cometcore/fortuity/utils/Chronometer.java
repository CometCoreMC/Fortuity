...
public class Chronometer {
    ...
    private static final Logger LOGGER = Fortuity.getLOGGER();
    ...
    private void runTask() {
        if (isAsync) {
            task = Bukkit.getScheduler().runTaskTimerAsynchronously(Fortuity.getInstance(), () -> {
                timeLeft--;
        ...
        task = Bukkit.getScheduler().runTaskTimer(Fortuity.getInstance(), () -> {
            timeLeft--;
            if (!isEndless && timeLeft <= 0) {
                onEnd.run();
                task.cancel();
